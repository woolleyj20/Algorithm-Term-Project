...
Our algorithm is similar to the theoretical optimum but can be improved. We start with a clique and have a loop that runs until we deconstruct the clique. We then have a loop that checks each person and a third loop that traverses their edges and assigns them as a visitor to the first free edge they see. The first loops runs (n - 1) + (n - 1)/(m - 1) + C times where C is the number of weeks added by having some number of couples. The second loop runs n times as it loops through the list of people. The third loop runs in worst case n times, but in practice runs faster than O(n). Our worst case time complexity is O(n^3) but we could improve our third loop if we found some sorting algorithm that sorted the edges by visitation status in O(logn) time. If this were achieved we would theoretically be at the optimal time complexity of O(n^2logn). 

The hardships of this project was finding the optimal solution and dealing with couples. Our inital solution was greedy and solved the problem but took a lot of weeks because our solution left people towards the bottom of the list constantly matching up with each other for most of the algorithm. Our solution to this problem was to move the head of the list to the bottom after every iteration to ensure that a new member would be the first one to pick every week. We also ran into trouble in situations where we filled all houses to max capacity but had one couple left to assign to the groups. Our initial algorithm allowed for houses to go one over capacity if we had any extra people left over but it did not allow two. We simply added another assignment loop after the initial assignments were done, which put the couples in a house at two over capacity. This did not make our time complexity worse since O(n + n) is still considered O(n). 
